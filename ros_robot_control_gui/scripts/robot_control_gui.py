...
class RobotControlGUI(Node):
    ...
    def _execute_one_by_one_mode(self, speed, accel, delay_ms):
        ...
        self.log_message("💡 보간 모드(0x03)를 사용하여 서보 기본 함수로 구동합니다 (동기화 모드 사용 안 함).")
        
        # 웨이포인트 모드 시작을 위해 마스터의 lastAngleSpeedMsgTime을 리셋
        # 첫 번째 메시지 전에 약간의 딜레이를 두어 이전 명령과 구분
        # 250ms 이상 지나면 마스터가 웨이포인트 모드로 인식하지 않으므로, 
        # 첫 번째 메시지를 보내기 전에 250ms 대기하여 마스터의 상태를 리셋
        self.log_message("⏳ 웨이포인트 모드 시작 대기 중...")
        time.sleep(0.25)  # 250ms 대기하여 마스터의 연속 전송 감지 상태 리셋
